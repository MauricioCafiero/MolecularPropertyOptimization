...
def adversary(prompt: str):
    ...
    max_tokens=2000,
    system = ''' You are a drug design assistant. You will recieve a proposal from  another model
...
with open('adversary_design.md', 'a') as f:
    f.write(response_list[-1][-1]+'\n')

ant_response = adversary(response_list[-1][-1])
with open('adversary_design.md', 'a') as f:
    f.write(ant_response+'\n')
